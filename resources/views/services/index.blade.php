...
<div class="container mt-4">
    ...
    <div class="row g-4">
                    ...
                    @auth
                        @if (Auth::id() !== $service->usuario_id)
                            ...
                            <button type="button" class="btn btn-primary w-100 fw-bold" 
                                    ...
                                    data-bs-target="#agendarModal-{{ $service->id }}">
                                Contratar e Agendar
                            </button>
        ...
        @auth
            @if (Auth::id() !== $service->usuario_id)
            <div class="modal fade" id="agendarModal-{{ $service->id }}" tabindex="-1" aria-labelledby="agendarModalLabel-{{ $service->id }}" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content" style="background-color: #1E293B; color: #ffffff;">
                        <div class="modal-header border-secondary">
                            <h5 class="modal-title text-info fw-bold" id="agendarModalLabel-{{ $service->id }}">Agendar Serviço</h5>
                            <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        
                        <form action="{{ route('orders.store', $service->id) }}" method="POST" novalidate>
                            @csrf
                            <div class="modal-body">
                                <p>Você está agendando: <strong>{{ $service->titulo }}</strong></p>
                                <p>Com: <strong>{{ $service->prestador->nome }}</strong></p>
                                
                                <hr class="border-secondary">

                                <div class="mb-3">
                                    <label for="data_agendamento-{{ $service->id }}" class="form-label">Escolha a data e hora do atendimento:</label>
                                    <input type="datetime-local" 
                                           class="form-control" 
                                           name="data_agendamento" 
                                           id="data_agendamento-{{ $service->id }}" 
                                ...
                                <small class="text-white-50 d-block">Cada serviço tem duração de 1 hora.</small>
                                <small class="text-warning d-block">Atenção: O horário de início deve ser entre 07:30 e 17:30.</small>
                                </div>
